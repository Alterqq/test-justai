...
const Favorites = () => {
  ...
  const favoriteBackground = useSelector(
    (state) => state.root.favoriteBackground
  );
  ...
  function handleDrop() {
    if (
      currentUser !== null &&
      !favorites.filter((u) => u.login.uuid === currentUser.login.uuid).length
    ) {
  ...
  return (
    <div
      className={`${styles.favoritesArea} ${
        favoriteBackground && styles.background
      }`}
    ...
    >
      ...
      {favorites.length !== 0 &&
        favorites.map((user) => (
          <FavoriteUserCard key={user.login.uuid} user={user} />
        ))}
      {!favorites.length && (
        <div className={styles.emptyState}>
          Пока нет избранных пользователей
        </div>
      )}
    </div>
  );
