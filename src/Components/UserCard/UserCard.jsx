...
const UserCard = (props) => {
  ...
  const getFilteredName = (firstName, lastName) => {
    ...
    return <div className={styles.fullName}>
      ...
      <span>{fullName.substr(end, fullName.length)}</span></div>
  }
  return (
      <div
          draggable
        ...
        <div>
          <div>{getFilteredName(props.user.name.first, props.user.name.last)}, дата
            регистрации: {new Date(props.user.registered.date).toLocaleDateString()}
          </div>
